...
def runGroundMotionAnalysis(gmData, GM_fact, dt):
    ...
    for i in range(Nstep):
        ...
        if status != 0:
            ...
            status = ops.analyze(1, DtAnalysis)
        if status!=0:
            print("Analysis failed trying Bryoden Algorithm and Changing Integrator too....")
            tol = 1.e-8
            maxNumIter = 50
            ops.test("EnergyIncr", tol, maxNumIter)
...
getModel(buildingID=20, NBay=1, NStory=10, LBeam=4, LCol=4, sectionType='RCFiber', startCoor=(4+gap,0))
